    ...
    <div className="h-full overflow-y-auto p-6 space-y-6">
        ...
        <div className="flex gap-2">
          {nextStatuses.map((s) => (
            <button
              ...
              className="px-3 py-1.5 text-xs font-medium rounded-lg border border-border bg-secondary hover:bg-secondary/80 transition-colors disabled:opacity-50"
            >
              {updating ? '…' : `Mark ${s}`}
            </button>
          ))}
      ...
      <section className="space-y-2">
        ...
        <div className="bg-secondary/50 rounded-lg p-4 text-sm space-y-1">
          ...
          <p className="text-muted-foreground">Fee: {formatPrice(order.delivery_fee)}</p>
        </div>
      </section>
